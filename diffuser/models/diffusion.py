...
class BehaviorCloningDiffusion(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, n_timesteps=1000,
                 ...
                 condition_guidance_w=0.1, ar_inv=False, train_only_inv=False, goals_condition=False,
                 bisimulation_model=None, return_based_s2g_model=None, goal_distance=None,
                 general_goal_prediction=False, diffusion_goal_prediction=False, return_type=None,
                 normed_zero_reward_value=0, srs_inv_model=False,
                 ):
        super().__init__()
        ...
        self.normed_zero_reward_value = normed_zero_reward_value
        self.srs_inv_model = srs_inv_model
        self.condition_guidance_w = condition_guidance_w
        assert general_goal_prediction + diffusion_goal_prediction == 1
        ## get loss coefficients and initialize objective
        # loss_weights = self.get_loss_weights(loss_discount)
        self.loss_fn = Losses['bc_l2']()

    def loss(self, trajectories):
        actions = trajectories[:, :, :self.action_dim]
        states = trajectories[:, :, self.action_dim:]

        pred_actions = self.model(states)
        loss, info = self.loss_fn(pred=pred_actions, targ=actions)
        return loss, info
...
class GaussianInvDynDiffusion(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, n_timesteps=1000,
        ...
        if self.ar_inv:
            ...
            if return_type == 7:
                inv_model_input_dim = 2 * self.observation_dim + 1  if srs_inv_model else 2 * self.observation_dim
            else:
                inv_model_input_dim = 2 * self.observation_dim
            self.inv_model = nn.Sequential(
                nn.Linear(inv_model_input_dim, hidden_dim),
                nn.ReLU(),
        ...
        if goals_condition:
            if general_goal_prediction:
                self.goal_prediction_model = return_based_s2g_model
            else:
                self.goal_prediction_model = None
        #     self.bisimulation_model = bisimulation_model

        self.returns_condition = returns_condition
        self.condition_guidance_w = condition_guidance_w
        self.goal_distance = goal_distance
        self.goals_condition = goals_condition
        assert self.returns_condition + self.goals_condition < 2
    ...
    def p_mean_variance(self, x, cond, t, returns=None, goals=None, tf_start_pos=0):
        if self.returns_condition:
            # epsilon could be epsilon or x0 itself
            epsilon_cond = self.model(x, cond, t, returns, use_dropout=False)
            epsilon_uncond = self.model(x, cond, t, returns, force_dropout=True)
            epsilon = epsilon_uncond + self.condition_guidance_w*(epsilon_cond - epsilon_uncond)
        elif self.goals_condition:
            epsilon_cond = self.model(x, cond, t, returns=returns, goals=goals, use_dropout=False)
            epsilon_uncond = self.model(x, cond, t, returns=returns, goals=goals, force_dropout=True)
            epsilon = epsilon_uncond + self.condition_guidance_w * (epsilon_cond - epsilon_uncond)
        else:
            epsilon = self.model(x, cond, t)
    ...
    @torch.no_grad()
    def p_sample(self, x, cond, t, returns=None, goals=None, tf_start_pos=0):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(x=x, cond=cond, t=t, returns=returns, goals=goals)
        noise = 0.5*torch.randn_like(x)
        # no noise when t == 0
        nonzero_mask = (1 - (t == 0).float()).reshape(b, *((1,) * (len(x.shape) - 1)))
        return model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond, returns=None, goals=None, tf_start_pos=0, verbose=True, return_diffusion=False):
        device = self.betas.device

        batch_size = shape[0]
        x = 0.5*torch.randn(shape, device=device)
        x = apply_conditioning(x, cond, 0, slide_mode=True if self.return_type == 8 or self.return_type == 7 else False,
                               sr_sequence=True if self.return_type == 7 else False)
        ...
        for i in reversed(range(0, self.n_timesteps)):
            timesteps = torch.full((batch_size,), i, device=device, dtype=torch.long)
            x = self.p_sample(x=x, cond=cond, t=timesteps, returns=returns, goals=goals, tf_start_pos=tf_start_pos)
            x = apply_conditioning(x, cond, 0, slide_mode=True if self.return_type == 8 or self.return_type == 7 else False,
                                   sr_sequence=True if self.return_type == 7 else False)
    ...
    @torch.no_grad()
    def conditional_sample(self, cond, returns=None, goals=None, horizon=None, timestep=None, tf_start_pos=0, *args, **kwargs):
        '''
            conditions : [ (time, state), ... ]
        '''
        device = self.betas.device
        try:
            for cond_key, cond_val in cond.items():
                batch_size = len(cond_val)
        except:
            raise Exception("cond is not dict !!! or cond contains more than one item !!!")
        horizon = horizon or self.horizon
        shape = (batch_size, horizon, cond_val.shape[-1])
        if self.goals_condition:
            if goals is None:
                if self.diffusion_goal_prediction:
                    uncondition_goal = torch.zeros(size=(batch_size, self.observation_dim)).to(device)
                    samples = self.p_sample_loop(shape=shape, cond=cond, returns=returns, goals=uncondition_goal, *args, **kwargs)
                    goals = samples[:, 1, :]
                else:
                    if timestep is not None:
                        return_goal_scale = np.minimum(0.4 + 0.5 / 100 * timestep, 0.9)
                    else:
                        return_goal_scale = 0.9
                    goal_returns = to_device(return_goal_scale * torch.ones(batch_size, 1), device)
                    goals = self.goal_prediction_model.get_state_to_goal(state=cond[0], reward=goal_returns)

        return self.p_sample_loop(shape=shape, cond=cond, returns=returns, goals=goals, tf_start_pos=tf_start_pos, *args, **kwargs)
    #------------------------------------------ training ------------------------------------------#
    ...
    def p_losses(self, x_start, cond, t, returns=None, goals=None):
        noise = torch.randn_like(x_start)

        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        x_noisy = apply_conditioning(x_noisy, cond, 0, slide_mode=True if self.return_type == 8 or self.return_type == 7 else False,
                                     sr_sequence=True if self.return_type == 7 else False)

        x_recon = self.model(x_noisy, cond, t, returns, goals=goals)

        if not self.predict_epsilon:
            x_recon = apply_conditioning(x_recon, cond, 0, slide_mode=True if self.return_type == 8 or self.return_type == 7 else False,
                                         sr_sequence=True if self.return_type == 7 else False)
    ...
    def loss(self, x, cond, returns=None, goals=None, observations=None, next_observations=None, bisimulation_rewards=None, state_predict_x=None, *args):
        if self.train_only_inv:
            ...
            batch_size = len(x)
            t = torch.randint(0, self.n_timesteps, (batch_size,), device=x.device).long()
            # # todo slidable diffusion window return to go
            ...
            if self.return_type == 8:
                ...
                x_t = x[:, history_length:-1, self.action_dim:]
                a_t = x[:, history_length:-1, :self.action_dim]
                x_t_1 = x[:, history_length+1:, self.action_dim:]
            elif self.return_type == 7:
                history_length = 0
                for key, val in cond.items():
                    history_length = key + 1
                x_t = x[:, history_length:-1, self.action_dim:]  if self.srs_inv_model else x[:, history_length:-1, self.action_dim:-1]
                a_t = x[:, history_length:-1, :self.action_dim]
                x_t_1 = x[:, history_length+1:, self.action_dim:-1]
            else:
                x_t = x[:, :-1, self.action_dim:]
                a_t = x[:, :-1, :self.action_dim]
                x_t_1 = x[:, 1:, self.action_dim:]
            x_comb_t = torch.cat([x_t, x_t_1], dim=-1)
            if self.return_type == 7:
                x_comb_t = x_comb_t.reshape(-1, 2 * self.observation_dim + 1) if self.srs_inv_model else x_comb_t.reshape(-1, 2 * self.observation_dim)
            else:
                x_comb_t = x_comb_t.reshape(-1, 2 * self.observation_dim)
            a_t = a_t.reshape(-1, self.action_dim)
            if self.ar_inv:
