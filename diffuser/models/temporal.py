...
class TemporalUnet(nn.Module):
    ...
    def __init__(
        ...
        if self.returns_condition:
            ...
            self.returns_mlp = nn.Sequential(
                        nn.Linear(returns_mlp_input_dim, dim),
                        act_fn,
                        nn.Linear(dim, dim * 4),
                        act_fn,
                        nn.Linear(dim * 4, dim),
            ...
            self.goals_returns_mlp = nn.Sequential(
                nn.Linear(goal_dim, dim),
                act_fn,
                nn.Linear(dim, dim * 4),
                act_fn,
                nn.Linear(dim * 4, dim),
    ...
    def forward(self, x, cond, time, returns=None, goals=None, use_dropout=True, force_dropout=False):
        '''
        ...
        if self.goals_condition:
            ...
            goals_embed = self.goals_returns_mlp(goals)
            if use_dropout:
    ...
    def get_pred(self, x, cond, time, returns=None, goals=None, use_dropout=True, force_dropout=False):
        '''
            x : [ batch x horizon x transition ]
            returns : [batch x horizon]
        '''
