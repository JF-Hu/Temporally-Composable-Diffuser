...
class MuJoCoRenderer:
    ...
    def pad_observations(self, observations):
        ...
        xpos = np.cumsum(xvel) * self.env.dt * 25
    ...
    def special_composite(self, plt_fig_path, paths, dim=(1024, 256), **kwargs):
        ...
        if plt_fig_path is not None:
            plt.figure(50*10)
            plt.imshow(images)
            plt.savefig(plt_fig_path, dpi=500)
            print(f'Saved {len(paths)} samples to: {plt_fig_path}')
            plt.close()

        return images

    def composite(self, savepath, paths, plt_fig_path=None, dim=(1024, 256), **kwargs):
        ...
        if savepath is not None:
            ...
            plt_savepath = savepath.split("/")
            file_name = plt_savepath[-1]
            if plt_fig_path is not None:
                plt_savepath[0] = plt_fig_path
            plt_savepath.insert(0, "analysis/plt_fig")
            plt_savepath = "/".join(plt_savepath[:-1])
            self.check_path(plt_savepath)
            plt.savefig(os.path.join(plt_savepath, file_name), dpi=500)
            print(f'Saved {len(paths)} samples to: {plt_savepath}')
            plt.close()
