...
class Config(collections.Mapping):
    ...
    def __init__(self, _class, verbose=True, savepath=None, device=None, father_save_path=None, **kwargs):
        ...
        if savepath is not None:
            if father_save_path is not None:
                if not os.path.exists(father_save_path):
                    os.mkdir(father_save_path)
                with open(os.path.join(father_save_path, savepath), "wb") as f:
                    pickle.dump(self, f)
            else:
